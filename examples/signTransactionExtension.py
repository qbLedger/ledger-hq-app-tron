...
# P1 = P1_FIRST = 0x00
apduMessage = "E0040000" + '{:02x}'.format(
    int(len(donglePath) / 2) + 1 +
    int(len(transactionRawFIRST) / 2)) + '{:02x}'.format(
        int(len(donglePath) / 4 / 2)) + donglePath + transactionRawFIRST
result = dongle.exchange(bytearray.fromhex(apduMessage))
# P1 = P1_LAST = 0x90
apduMessage = "E0049000" + '{:02x}'.format(int(
    len(transactionRawEND) / 2)) + transactionRawEND
result2 = dongle.exchange(bytearray.fromhex(apduMessage))

if (binascii.hexlify(result1[0:65]).decode() == binascii.hexlify(
        result2[0:65]).decode()):
...
# P1 = P1_LAST = 0x90
apduMessage = "E0049000" + '{:02x}'.format(int(
    len(transactionBIG4) / 2)) + transactionBIG4
